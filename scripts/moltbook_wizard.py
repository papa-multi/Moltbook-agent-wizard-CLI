...
def _load_missing_mint_queue():
    if not os.path.exists(MISSING_MINT_QUEUE_PATH):
        return []
    try:
        with open(MISSING_MINT_QUEUE_PATH, "r", encoding="utf-8") as f:
            data = json.load(f)
    except (OSError, json.JSONDecodeError):
        return []
...
def _save_missing_mint_queue(queue):
    os.makedirs(PROFILE_DIR, exist_ok=True)
    with open(MISSING_MINT_QUEUE_PATH, "w", encoding="utf-8") as f:
        json.dump(queue, f, indent=2, sort_keys=True)
    os.chmod(MISSING_MINT_QUEUE_PATH, 0o600)
...
def _enqueue_missing_mint(content_id, reason, delay_minutes=10):
    ...
    queue.append(entry)
    _save_missing_mint_queue(queue[-200:])
...
def _process_missing_mint_queue(max_items=3):
    ...
    for entry in queue:
        ...
        if data:
            ...
            new_queue.append(entry)
        processed += 1
    _save_missing_mint_queue(new_queue[-200:])
...
def _post_verify(base_url, api_key, payload):
    headers = {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}
    try:
        resp = requests.post(f"{base_url}/verify", headers=headers, json=payload, timeout=30)
    except requests.RequestException as exc:
...
def _handle_verification(base_url, api_key, data):
    ...
    if challenge:
        ...
        if auto_answer:
            ...
            verify_data, verify_error = _post_verify(base_url, api_key, payload)
            if verify_data:
                ...
                if content_id:
                    ...
                    _record_content_id(content_id, label=state.get("profile_name"), content_type=verify_data.get("content_type"))
                _update_verification_status(code, "verified", answer=auto_answer)
            else:
                content_id = _content_id_from_error(verify_error)
                if content_id:
                    print(f"\"content_id\": \"{content_id}\"")
                    _record_content_id(content_id, label=state.get("profile_name"), content_type="post")
                if verify_error:
                    print(verify_error)
...
def _cmd_verify(base_url, state):
    ...
    def _submit_verification(code, challenge):
        ...
        data, error = _post_verify(base_url, api_key, payload)
        if data:
            _pretty_json(data)
            content_id = data.get("content_id")
            if content_id:
                print(f"\"content_id\": \"{content_id}\"")
                _record_content_id(content_id, label=state.get("profile_name"), content_type=data.get("content_type"))
...
def _fetch_agent_holdings(agent_name):
    ...
    if html_text is None:
        html_text, curl_error = _curl_fetch_html(url)
        if html_text is None:
            return None, error or f"curl failed: {curl_error}"
    holdings = _extract_holdings_html(html_text)
    if not holdings:
...
def _menu():
    ...
    print("10) Auto-mint every 30 minutes (all accounts)")
...
def _cmd_auto_mint():
    _print_header("Auto-mint every 30 minutes (all accounts)")
    content = _prompt_multiline(
        "Paste mbc20.xyz + JSON, or just the JSON inscription"
    )
    if not content:
    ...
    if not tick or not amount:
        ...
        return
    print(f"Auto-mint configured: tick={tick} amt={amount} interval=30m profiles=all")
    submolt = "mbc20"
    prefix = "mbc20.xyz"
    interval = "30"
